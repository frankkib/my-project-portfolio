from flask import Blueprint, render_template, request, session, redirect, url_for, jsonify, flash, send_from_directory
...
def dashboard():
    ...
    form = HouseForm()
    return render_template('dashboard.html', user=user, houses=houses, form=form)
...
@house_bp.route('/upload/<filename>')
def upload(filename):
